<template>
  <div class="w-screen h-screen  ">
    ...
    <div class="w-screen h-screen pl-32 bg-slate-800">
      ...
      <div class=" w-11/12 h-screen bg-gray-400 flex pl-36 rounded-2xl">
        <div class="m-auto "
        :class="gameStr == true ? 'cursor-crosshair' : ''">
        <div class="text-emerald-500 text-4xl font-bold flex flex-col  ">
          ...
          <div v-if="!startBtn" class="flex w-28 h-16 rounded-2xl  bg-white mt-10">
            <button v-if="!startBtn" class="btn border-none btn-xl bg-black text-slate-50 m-auto w-24"
              @click="start">CROSSHAIR</button>
          </div>
          <div class="flex  " v-if="startBtn && !gameStr">
        ...
        <div v-if="gameStr && !gameOver" v-show="enemy" class="bg-emerald-500 rounded-full" style="position: absolute;"
